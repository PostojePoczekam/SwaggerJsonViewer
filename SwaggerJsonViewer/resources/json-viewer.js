...
var JSONViewer = (function (document) {
	...
	function walkJSONTree(outputParent, value, maxLvl, colAt, lvl) {
		...
		if (typeof realValue === "object" && realValue !== null && !isDate) {
			...
			if (lvl === 0) {
				...
				rootLink.appendChild(_createBracket(isArray ? "[" : "{"));
				...
				items.forEach(function (key, ind) {
					...
					if (typeof item === "object") {
						...
						if (!item || item instanceof Date) {
							...
							li.appendChild(_createPunctuation(isArray ? "" : ": "))
						...
						// array & object
							...
							if (!itemLen) {
								...
								li.appendChild(_createPunctuation(": "));
								li.appendChild(_createBracket(itemIsArray ? "[]" : "{}"));
							}
							else {
								// 1+ items
								...
								if (maxLvl >= 0 && lvl + 1 >= maxLvl) {
									...
									li.appendChild(_createPunctuation((typeof key === "string") ? ": " : ""));
									li.appendChild(_createBracket(itemIsArray ? "[" : "{"));
								}
								else {
									itemLink.appendChild(_createPunctuation((typeof key === "string") ? ": " : ""));
									itemLink.appendChild(_createBracket(itemIsArray ? "[" : "{"));
			...
			if (lvl === 0) {
				...
				// root cover
				outputParent.appendChild(_createBracket(isArray ? "]" : "}"));
	...
	function _createBracket(bracket) {
	...
	function _createPunctuation(mark) {
		var punctuationEl = document.createElement("span");
		punctuationEl.classList.add("punctuation");
		punctuationEl.innerHTML = mark;
